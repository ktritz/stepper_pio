...
class Stepper:
    ...
    def _setup_sm(self):
        ...
        self._sm.background_write(array.array("L", [0]))
        self.limit = 0
    ...
    def enable(self):
        if not self.enable_pin:
            return None
        return self.enable_pin.value ^ (self.enable_active == "HIGH")

    @enable.setter
    def enable(self, value):
        if not self.enable_pin:
            return None
        self.enable_pin.value = bool(value) ^ (self.enable_active == "HIGH")
    ...
    def velocity(self, value):  # accelerate to contiuous rotation
        ...
        if start_velocity == value:
            return
        _ = self.steps  # flush counter
        if value == 0:
            self.stop()
            return
        if value * start_velocity > 0:  # increase/decrease speed
            self._change_velocity(start_velocity, value)
            self._sm.clear_txstall()
            return
        if value * start_velocity < 0:  # changing direction
            self.stop()
            while self.velocity:
                time.sleep(0.01)
            start_velocity = 0
        ...
        if start_velocity == 0:
            self.steps = 1 - 2 * self.direction  # sometimes setting velocity glitches
        value = abs(value)
        acc_delays = self.delays._change_velocity(start_velocity, value)
        full_speed = array.array("L", [((acc_delays[-1] >> 16) << 16) + self._fd])
        self._sm.background_write(once=acc_delays, loop=full_speed)
        self._sm.clear_txstall()

    def _change_velocity(self, start_velocity, value):
        value = abs(value)
        acc_delays = self.delays._change_velocity(start_velocity, value)
        full_speed = array.array("L", [((acc_delays[-1] >> 16) << 16) + self._fd])
        self._sm.background_write(once=acc_delays, loop=full_speed)
    ...
    def steps(self):
        if self.counter:
            sign = 1 - 2 * self.direction
            if self.velocity != 0:
                return (self._steps + sign * self.counter.count) / self.micro_steps
            else:
    ...
    def stop(self):  # gracefully decelerate
        ...
        self._sm.background_write(once=dec_delays)
        self._sm.clear_txstall()
